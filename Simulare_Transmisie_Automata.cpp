...
#define BUTON D2
#define PADELA_SUS D3
#define PADELA_JOS D4
...
bool starePadelaSus = false;
bool starePadelaJos = false;
...
void citesteButon() {
  ...
  if (acum - ultimDebounce > 50) {
    if (citire == LOW && !butonApasat) {
      ...
      butonApasat = false;
      if (acum - timpApasare >= 1000) {
        if (vitezaSmooth < 1.0) modCurent = P;
      } else {
        modCurent = N;
        modManual = false;
      }
...
void citestePadele() {
  if (modCurent != D) return;
  bool citireSus = (digitalRead(PADELA_SUS) == LOW);
  bool citireJos = (digitalRead(PADELA_JOS) == LOW);
  if (citireSus && !starePadelaSus) {
    if (treaptaCurenta < MAX_TREAPTA) {
      float viitorRPM = rpmSmooth * (RAPORT_CUTIE[treaptaCurenta + 1] / RAPORT_CUTIE[treaptaCurenta]);
      if (viitorRPM > 1000) {
        treaptaCurenta++;
        rpmSmooth = viitorRPM;
        modManual = true;
  ...
  starePadelaSus = citireSus;
  starePadelaJos = citireJos;
}
void calculeaza() {
  unsigned long acum = millis();
  ...
  switch (modCurent) {
    ...
    case D: {
      ...
      if (!modManual && acum - ultimaSchimbare >= 900) {
...
void actualizeazaLCD() {
  ...
  lcd.setCursor(0, 0);
  char buf[17];
  char modC = (modCurent == P) ? 'P' : (modCurent == R) ? 'R' : (modCurent == N) ? 'N' : (modManual ? 'M' : 'D');
  char modS = (modCurent == D && !modManual) ? ((modCondus == 0) ? 'C' : (modCondus == 1) ? 'S' : 'K') : ' ';
...
void setup() {
  ...
  pinMode(PADELA_SUS, INPUT_PULLUP);
  pinMode(PADELA_JOS, INPUT_PULLUP);
  rpmSmooth = 750.0;
  vitezaSmooth = 0.0;
  treaptaCurenta = 1;
